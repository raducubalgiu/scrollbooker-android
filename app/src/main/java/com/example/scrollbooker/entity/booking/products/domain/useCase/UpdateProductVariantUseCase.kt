...
class UpdateProductVariantUseCase @Inject constructor(
    private val repository: ProductRepository
) {
    suspend operator fun invoke(productId: Int, variantId: Int, request: ProductVariantRequest): Result<Product> {
        return runSuspendCatching {
            repository.updateVariant(productId, variantId, request)
        }
    }
}
